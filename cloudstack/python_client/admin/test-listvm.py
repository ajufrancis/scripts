...
# domainid= 9b832394-f254-4f6d-ba67-ed3979ac9fc6
    formatted_url = hu.format_url(command=command, option_list=options,url=url, apikey=apikey, secretkey=secretkey)
    print formatted_url
#    try:
#        response = urllib2.urlopen(formatted_url).read()
#        reponse = response.decode('UTF-8').encode('gb2312')
#        root = ElementTree.fromstring(response)
#        for vm in root.findall('virtualmachine'):
#	    print type(vm)
#	    name = vm.find('instancename')
#	    disname = vm.find('displayname')
#	    zonename = vm.find('zonename')
#
##	    print name,disname,zonename
#
#    except Exception, e:
#        print 'Error !', e
