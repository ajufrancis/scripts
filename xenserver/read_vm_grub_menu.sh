...
if [ "${grub_conf}" = "" ]; then
   ...
   grub_conf_path=${mnt}${grub_conf}
   menuentry=$(egrep 'menuentry|vmlinuz|initrd.img' $grub_conf_path)
   echo $menuentry
fi
}
...
function usage {
    ...
    echo
    echo "Either -u or -n must be supplied to specify the VM to read"
    exit 1
}
