...
elsif($test_name =~ m/^system-temp$/i){
...
elsif($test_name =~ m/^volt_minus12$/i){
...
sub result_validation{
  ...
  #Validate
  if (((int($oc_low) != 0)&&(abs($data_fan) < $oc_low))||((int($oc_high) != 0)&&(abs($data_fan) > $oc_high))) {
     $result = $ERRORS{"CRITICAL"};
     $data_text .= $label_fan.": ".$data_fan." ".$unit." (CRIT); ";
  } elsif (((int($ow_low) != 0)&&(abs($data_fan) < $ow_low))||((int($ow_high) != 0)&&(abs($data_fan) > $ow_high))) {
